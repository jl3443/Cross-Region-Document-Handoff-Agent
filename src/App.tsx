        ...
        {showDraftPanel && activeDrafts.length > 0 && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 right-0 z-50 w-[480px] border-l border-neutral-800 shadow-2xl"
            >
              ...
              <div className="w-full max-w-md rounded-2xl bg-[#111111] border border-neutral-800 shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between border-b border-neutral-800 px-6 py-4">
                  <div className="flex items-center gap-2.5">
                    ...
                    <h3 className="text-sm font-semibold text-white">Compliance Override Review</h3>
                  </div>
                  <button onClick={() => setShowComplianceModal(false)} className="rounded-md p-1.5 text-neutral-500 hover:bg-neutral-800 hover:text-white transition-colors">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                <div className="p-6 space-y-4">
                  <div className="rounded-lg border border-purple-500/20 bg-purple-500/5 p-4">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-purple-400 mb-1">Exception</p>
                    <p className="text-sm font-semibold text-white">{complianceOverride.documentName}</p>
                  ...
                  <div className="space-y-1">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-neutral-500 mb-2">Approval Steps</p>
                    {[
                      ...
                      <div key={i} className="flex items-center gap-3 py-2">
                        <div className={`h-5 w-5 rounded-full flex items-center justify-center ring-1 ${
                          step.done ? 'bg-emerald-500/10 ring-emerald-500/20' : 'bg-neutral-800 ring-neutral-700'
                        }`}>
                          {step.done ? (
                        ...
                        <span className={`text-xs ${step.done ? 'text-emerald-400 font-medium' : 'text-neutral-400'}`}>
                          {step.label}
                        </span>
                      </div>
                    ))}
                  ...
                  {complianceStatus === 'pending' && (
                    <button
                      className="w-full rounded-lg bg-purple-500 px-4 py-2.5 text-xs font-semibold text-white hover:bg-purple-600 transition-colors"
                      onClick={() => {
                  ...
                  {complianceStatus === 'reviewing' && (
                    <div className="flex items-center justify-center gap-2 py-2 text-xs text-purple-400">
                      <div className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-purple-800 border-t-purple-400" />
                      Compliance team reviewing...
                    </div>
                  )}
                  {complianceStatus === 'approved' && (
                    <div className="flex items-center justify-center gap-2 py-2 text-xs font-semibold text-emerald-400">
        ...
        {showDocPack && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 z-50 flex items-center justify-center p-4"
            >
              <div className="w-full max-w-lg rounded-2xl bg-[#111111] border border-neutral-800 shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between border-b border-neutral-800 px-6 py-4">
                  <div className="flex items-center gap-2.5">
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500/10 ring-1 ring-emerald-500/20">
                      <Package className="h-4 w-4 text-emerald-400" />
                    </div>
                    <h3 className="text-sm font-semibold text-white">Final Document Pack</h3>
                  </div>
                  <button onClick={() => setShowDocPack(false)} className="rounded-md p-1.5 text-neutral-500 hover:bg-neutral-800 hover:text-white transition-colors">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                <div className="p-6 space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-neutral-500 mb-2">Included Documents</p>
                    {scenario.documents.map((doc) => {
                      const isValid = doc.status === 'validated';
                      return (
                        <div key={doc.id} className="flex items-center gap-3 rounded-lg border border-neutral-800 bg-neutral-900/50 px-3 py-2.5">
                          <FileCheck2 className={`h-4 w-4 shrink-0 ${isValid ? 'text-emerald-400' : 'text-amber-400'}`} />
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium text-white truncate">{doc.name}</p>
                            {doc.fileName && (
                              <p className="text-[10px] text-neutral-600">{doc.fileName}</p>
                            )}
                          </div>
                          <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-md ring-1 ${isValid ? 'bg-emerald-500/10 text-emerald-400 ring-emerald-500/20' : 'bg-amber-500/10 text-amber-400 ring-amber-500/20'}`}>
                  ...
                  {scenario.hazmat && (
                    <div className="rounded-lg border border-orange-500/20 bg-orange-500/5 p-3 flex items-start gap-2.5">
                      <Flame className="h-4 w-4 text-orange-400 mt-0.5 shrink-0" />
                      <div>
                        <p className="text-xs font-semibold text-orange-400">DG Compliance Verified</p>
                  ...
                  <button
                    className="w-full rounded-lg bg-white px-4 py-2.5 text-xs font-semibold text-black hover:bg-neutral-200 transition-colors inline-flex items-center justify-center gap-2"
                    onClick={() => {
                      setShowDocPack(false);
