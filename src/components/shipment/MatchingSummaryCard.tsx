...
export function MatchingSummaryCard({ summary }: MatchingSummaryCardProps) {
  const stats = [
    ...
    },
    {
      ...
      icon: AlertTriangle,
      valueClass:
        summary.exceptionsDetected > 0 ? 'text-amber-400' : 'text-white',
    },
    {
      ...
      icon: XOctagon,
      valueClass:
        summary.blockingIssues > 0 ? 'text-red-400' : 'text-white',
    },
  ];
    ...
    <Card title="Matching Summary">
      <div className="grid grid-cols-2 gap-3">
        {stats.map(({ label, value, icon: Icon, valueClass }) => (
          <div
            key={label}
            className="flex items-start gap-3 rounded-lg bg-neutral-800/50 border border-neutral-700/50 px-3 py-2.5"
          >
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-neutral-900 ring-1 ring-neutral-700">
              <Icon size={16} className="text-neutral-400" />
            </div>
            <div>
              <p className={cn('text-lg font-semibold leading-tight', valueClass)}>
                {value}
              </p>
