...
export function MatchingSummaryCard({ summary }: MatchingSummaryCardProps) {
  const stats = [
    ...
    },
    {
      ...
      icon: AlertTriangle,
      valueClass:
        summary.exceptionsDetected > 0 ? 'text-amber-600' : 'text-slate-900',
    },
    {
      ...
      icon: XOctagon,
      valueClass:
        summary.blockingIssues > 0 ? 'text-red-600' : 'text-slate-900',
    },
  ];
    ...
    <Card title="Matching Summary">
      <div className="grid grid-cols-2 gap-3">
        {stats.map(({ label, value, icon: Icon, valueClass }) => (
          <div
            key={label}
            className="flex items-start gap-3 rounded-md bg-slate-50 px-3 py-2.5"
          >
